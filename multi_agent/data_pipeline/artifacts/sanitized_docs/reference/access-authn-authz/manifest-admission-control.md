...
> **Note:** If two manifest files define objects of the same type with the same name, the
...
> **Note:** Webhook URLs must be reachable from the kube-apiserver at startup. Only
...
> **Caution:** If an invalid manifest file is present at startup, the API server does not
...
> **Warning:** Downgrading without removing the `staticManifestsDir` configuration causes the
