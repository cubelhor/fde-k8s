...
*(Feature State: stable as of v1.10)*
...
> **Note:** Preflight checks can be invoked individually with the
...
> **Caution:** The generated configuration files include an embedded authentication key, and you should treat
...
> **Note:** TLS bootstrapping for nodes can be configured with the command
...
> **Note:** The access to the `cluster-info` ConfigMap _is not_ rate-limited. This may or may not be a
...
> **Note:** This phase can be invoked individually with the command
...
> **Note:** You can skip CA validation by passing the `--discovery-token-unsafe-skip-ca-verification` flag on the command line.
...
> **Note:** - The temporary authentication is validated against the token saved during the `kubeadm init`
