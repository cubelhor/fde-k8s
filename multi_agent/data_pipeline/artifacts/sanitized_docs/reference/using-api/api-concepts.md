...
> **Note:** Clients that receive a response in `application/vnd.kubernetes.protobuf` that does
...
Kubernetes  introduces an alpha feature that allows clients
...
[`ListOptions`](/docs/reference/generated/kubernetes-api//#listoptions-v1-meta)
...
> **Note:** A list response that contains `shardInfo` represents a filtered subset of the full
...
> **Note:** Keep in mind that the Kubernetes API does not have a `kind` named `List`.
...
> **Note:** `as=PartialObjectMetadata` should be used in specific resource requests, and `as=PartialObjectMetadataList` should be used for lists.
...
> **Caution:** This may break the workload associated with the resource being force deleted, if it
...
> **Note:** The user performing the force **delete** operation must have the privileges to do both
...
> **Note:** If the user issues a **delete** request with `ignoreStoreReadErrorWithClusterBreakingPotential`
...
Starting from v1.33, Kubernetes (including v) offers a way to define field validations using _declarative tags_.
...
> **Note:** If a webhook actually does have side effects, then the `sideEffects` field should be
...
> **Note:** The Kubernetes _server side apply_ mechanism has superseded Strategic Merge
  Patch.
...
> **Note:** If your cluster's API server does not honor the `resourceVersionMatch` parameter,
