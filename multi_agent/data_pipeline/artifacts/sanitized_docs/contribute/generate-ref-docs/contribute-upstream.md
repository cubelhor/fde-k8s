...
Kubernetes version , and you want to backport your change into the
release- branch.
...
commit into the release- branch. The idea is to cherry pick the commit
...
release- branch, the next step is to run these scripts in the
release- branch of your local environment.
...
At this point, both the master branch and the release- branch have your updated `types.go`
file and a set of generated files that reflect the change you made to `types.go`. Note that the
generated OpenAPI spec and other generated files in the release- branch are not necessarily
the same as the generated files in the master branch. The generated files in the release- branch
contain API elements only from Kubernetes . The generated files in the master branch might contain
API elements that are not in , but are under development for .
