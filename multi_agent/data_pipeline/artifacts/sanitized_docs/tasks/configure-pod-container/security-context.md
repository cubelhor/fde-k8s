...
> **Note:** _Implicitly merged_ supplementary groups may cause security problems particularly when accessing
...
> **Note:** Please note that the values in the `status.containerStatuses[].user.linux` field is _the first attached_
...
> **Note:** At this alpha release(from v1.31 to v1.32), when a pod with `SupplementalGroupsPolicy=Strict` are scheduled to a node that does NOT support this feature(i.e. `.status.features.supplementalGroupsPolicy=false`), the pod's supplemental groups policy falls back to the `Merge` policy _silently_.
...
[SecurityContext](/docs/reference/generated/kubernetes-api//#securitycontext-v1-core) object.
...
[SeccompProfile](/docs/reference/generated/kubernetes-api//#seccompprofile-v1-core) object consisting of `type` and `localhostProfile`.
...
is a
[AppArmorProfile](/docs/reference/generated/kubernetes-api//#apparmorprofile-v1-core) object consisting of `type` and `localhostProfile`.
...
[SELinuxOptions](/docs/reference/generated/kubernetes-api//#selinuxoptions-v1-core)
...
> **Note:** Kubernetes v1.27 introduced an early limited form of this behavior that was only applicable
...
> **Warning:** We strongly recommend clusters that use SELinux to enable this controller and make sure that
...
> **Note:** Setting `procMount` to Unmasked requires the `spec.hostUsers` value in the pod
...
> **Warning:** After you specify an MCS label for a Pod, all Pods with the same label can access the Volume.
...
* [PodSecurityContext](/docs/reference/generated/kubernetes-api//#podsecuritycontext-v1-core)
* [SecurityContext](/docs/reference/generated/kubernetes-api//#securitycontext-v1-core)
