...
> **Note:** Although rare, more than one Pod could be started for the same index (due to various reasons such as node failures,
...
> **Note:** If the Job failed, either by the Pod failure policy or Pod backoff
...
> **Note:** Starting with Kubernetes v1.28, when Pod failure policy is used, the Job controller recreates
...
> **Note:** When you specify both a success policy and some terminating policies such as `.spec.backoffLimit` and `.spec.podFailurePolicy`,
...
> **Note:** It is recommended to set `ttlSecondsAfterFinished` field because unmanaged jobs
 ...
 *(Feature State: stable as of v1.27)*
...
> **Note:** See [My pod stays terminating](/docs/tasks/debug/debug-application/debug-pods/) if you
...
> **Note:** When the [`WorkloadWithJob`](/docs/reference/command-line-tools-reference/feature-gates/)
...
> **Note:** When using this feature, make sure the controller indicated by the field is
installed, otherwise the Job may not be reconciled at all.

> **Note:** When developing an external Job controller be aware that your controller needs
