...
> **Note:** * When you `DELETE` an object, Kubernetes adds the deletion timestamp for that object and then
...
> **Note:** Custom finalizer names **must** be publicly qualified finalizer names, such as `example.com/finalizer-name`.
...
> **Note:** In cases where objects are stuck in a deleting state, avoid manually
