...
> **Note:** The graceful node shutdown feature depends on systemd since it takes advantage of
...
> **Note:** When pods were evicted during the graceful node shutdown, they are marked as shutdown.
...
> **Note:** The ability to take Pod priority into account during graceful node shutdown was introduced
...
> **Note:** - Before adding the taint `node.kubernetes.io/out-of-service`, it should be verified
...
> **Note:** - Caution must be taken while using the [Non-Graceful Node Shutdown](#non-graceful-node-shutdown) procedure.
