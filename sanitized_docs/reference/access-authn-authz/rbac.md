...
> **Note:** You cannot restrict **deletecollection** or top-level **create** requests by resource name.
...
> **Caution:** Using wildcards in resource and verb entries could result in overly permissive access being granted
...
> **Caution:** The control plane overwrites any values that you manually specify in the `rules` field of an
...
> **Caution:** The prefix `system:` is reserved for Kubernetes system use, so you should ensure
...
> **Note:** - `system:serviceaccount:` (singular) is the prefix for service account usernames.
...
> **Caution:** Take care when modifying ClusterRoles and ClusterRoleBindings with names
...
> **Note:** If you edit that ClusterRole, your changes will be overwritten on API server restart
...
<td>Allows access to the resources required by the scheduler component.</td>
</tr>
<tr>
...
<td>Allows access to the resources required by the controller manager component.
...
<td>Allows read access to control-plane monitoring endpoints (i.e. kube-apiserver liveness and readiness endpoints (<tt>/healthz</tt>, <tt>/livez</tt>, <tt>/readyz</tt>), the individual health-check endpoints (<tt>/healthz/*</tt>, <tt>/livez/*</tt>, <tt>/readyz/*</tt>),  <tt>/metrics</tt>), and causes the kube-apiserver to respect the traceparent header provided with requests for tracing. Note that individual health check endpoints and the metric endpoint may expose sensitive information.</td>
</tr>
</tbody>
</table>
...
> **Note:** Permissions given to the "default" service account are available to any pod
...
> **Caution:** Enabling this means the `kube-system` namespace contains Secrets
...
> **Warning:** This allows any application full access to your cluster, and also grants
