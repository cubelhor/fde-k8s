...
> **Note:** Kubernetes  does not support certificate _revocation_.
...
> **Note:** Machine identities for nodes are not the same as
ServiceAccounts.
...
> **Warning:** Because service account tokens can also be stored in Secret API objects, any user with
...
> **Note:** If you specify `--authentication-config` along with any of the `--oidc-*` command line arguments, this is
...
> **Note:** The Kubernetes API server defaults to sending `authentication.k8s.io/v1beta1` token reviews for backwards compatibility.
...
> **Warning:** If you have a certificate authority (CA) that is also used in a different context, **do not** trust
...
In Kubernetes , tokens last indefinitely, and the token list cannot be
...
> **Caution:** You should read the documentation about configuring an [authenticating proxy](#authenticating-proxy)
...
> **Note:** Modifications to this file should be done in an atomic way (for example: writing to a peer temporary file, then renaming the temporary file to replace this file).
 ...
 *(Feature State: stable as of v1.28)*
...
> **Note:** The Kubernetes API server fills `userInfo` after all authentication mechanisms are applied,
