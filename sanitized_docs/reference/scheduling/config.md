...
> **Note:** `score` extension point is enabled when `StorageCapacityScoring` feature is
...
> **Note:** Pod's scheduling events have `.spec.schedulerName` as their `reportingController`.
...
> **Note:** All profiles must use the same plugin in the `queueSort` extension point and have
