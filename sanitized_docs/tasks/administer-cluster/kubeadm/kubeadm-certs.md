...
*(Feature State: stable as of v1.15)*
...
> **Note:** `kubeadm` cannot manage certificates signed by an external CA.
...
> **Note:** On nodes created with `kubeadm init` from versions prior to kubeadm version 1.17, there is a
...
> **Warning:** Avoid sharing the `super-admin.conf` or `admin.conf` files. Instead, create least
...
> **Note:** This guide uses the default Kubernetes directory `/etc/kubernetes`, which requires
...
> **Note:** If you are using an external CA, you must generate the same files out of band and manually
...
> **Note:** You must process the `kubelet.conf.csr` file on the primary control plane node
...
> **Note:** If you are using external CA and already have CA serial number files (`.srl`) for
