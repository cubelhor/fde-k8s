...
> **Note:** You can make multiple changes to a paused Deployment. Kubernetes applies all
...
> **Note:** `maxUnavailable` and `maxSurge` accept an absolute number or a percentage.
...
> **Note:** The `CHANGE-CAUSE` column shows the value of the `kubernetes.io/change-cause`
...
> **Note:** A Deployment's revision history is stored in the ReplicaSets it controls.
