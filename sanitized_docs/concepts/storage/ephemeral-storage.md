...
> **Note:** The kubelet tracks `tmpfs` emptyDir volumes as container memory use, rather
than as local ephemeral storage.

> **Note:** The kubelet can only track ephemeral storage on the filesystems it observes
...
> **Caution:** If the kubelet is not measuring local ephemeral storage, then a Pod
...
> **Note:** In this mode, the kubelet does not track open file descriptors
for deleted files.
