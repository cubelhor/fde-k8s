...
> **Note:** If you want Kubernetes to successfully schedule the Pods in this example, you
must have existing nodes with the `kubernetes.io/os=linux` label.

#### Node affinity per scheduling profile

 *(Feature State: beta as of v1.20)*
...
> **Note:** The DaemonSet controller, which [creates Pods for DaemonSets](/docs/concepts/workloads/controllers/daemonset/#how-daemon-pods-are-scheduled),
...
> **Note:** Inter-pod affinity and anti-affinity require substantial amounts of
processing which can slow down scheduling in large clusters significantly. We do
not recommend using them in clusters larger than several hundred nodes.

> **Note:** Pod anti-affinity requires nodes to be consistently labeled, in other words,
 ...
 *(Feature State: stable as of v1.24)*
...
> **Note:** The `matchLabelKeys` field is a beta-level field and is enabled by default in
Kubernetes .
...
> **Caution:** It's not recommended to use `matchLabelKeys` with labels that might be updated directly on pods.
...
> **Note:** The `mismatchLabelKeys` field is a beta-level field and is enabled by default in
Kubernetes .
When you want to disable it, you have to disable it explicitly via the
`MatchLabelKeysInPodAffinity` [feature gate](/docs/reference/command-line-tools-reference/feature-gates/).
...
> **Caution:** It's not recommended to use `mismatchLabelKeys` with labels that might be updated directly on pods.
...
You can use _topology spread constraints_ to control how Pods
...
> **Note:** `Gt` and `Lt` operators will not work with non-integer values. If the given value
